...
db.define_table('projeto',
                ...
                Field('data_saida_venda', 'date', label="Data Saida Venda", default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
                Field('data_cobranca', 'date', label="Data Cobrança", default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
				Field('comissao_chefe_cobranca', 'double', label="% Comissão Cob Chefe",notnull=True, default=5),
                #info_chegada_Venda
				Field('devolucao_dinh_venda', 'double', label="Devol Dinh Venda",notnull=True, default=0),
                Field('diferenca_chefe', 'double', notnull=True, default=0, writable=False, readable=False),
                Field('data_chegada_venda', 'date', label="Data Chegada Venda", default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
                Field('ultima_cidade', 'string',label="Ultima Cidade"),
                Field('ultima_data_cobranca', 'date', label="Ultima Data Cobr", default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
				Field('recebido_chegada_venda', 'double', label="Recebido na chegada",notnull=True, default=0, writable=False, readable=False),
                #info_antes_cobranca
                Field('nome_cobrador', 'string', label="Nome Cobrador"),
				Field('comissao_cobrador', 'double', label="% Comissao Cobrador",notnull=True, default=4),
                Field('diferenca_cobrador', 'double', notnull=True, default=0, writable=False, readable=False),
				Field('vale_cobrador', 'double', label="Vale Cobrador",notnull=True, default=0),
				Field('adiantamento_dinh_cobranca', 'double', label="Adiant Dinh Cobr",notnull=True, default=0),
                Field('data_saida_cobrador', 'date', label="Data saida Cobrador", default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
                #info_chegada_venda
				Field('devolucao_dinh_cobranca', 'double', label="Devol Dinh Cobrança",notnull=True, default=0),
                Field('data_chegada_cobrador', 'date', label="Data Chegada Cobrador", default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
				Field('recebido_chegada_cobrac', 'double', label="Recebido na Chegadaa Cobrador", writable=False, readable=False,notnull=True, default=0),
...
db.define_table('deposito',
				...
				Field('data_deposito','date', default=request.now, requires = IS_DATE(format=('%d-%m-%Y'))),
				auth.signature)
